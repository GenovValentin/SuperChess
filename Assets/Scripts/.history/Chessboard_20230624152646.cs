...
#region  Initialization
    ...
    public void OnDrawButton()
    {
        if (!localGame)
        {
            ...
            DisplayOfferDraw(true);
            Invoke("DisplayOfferDraw", 3.0f);
            return;
        }
    ...
    private void DisplayOfferDraw(bool show = false)
    {
        offeredDraw.transform.gameObject.SetActive (show);
    }
...
#region
    ...
    private void OnDeclineClient(NetMessage msg)
    {
        ...
        if (dc.teamNr != (int) currentTeam)
        {
            ...
            DisplayDeclined(true);
            Invoke("DisplayDeclined", 3.0f);
            Invoke("DisplayDeclined2", 5.0f);
        }
        DisplayOfferDraw(false);
        drawButton.interactable = true;
        ResetPlayerDraw();
    }

    private void DisplayDeclined(bool show = false)
    {
        Debug.Log("DisplayDeclined");
        declinedTMP.SetActive (show);
    }

    private void DisplayDeclined2()
    {
        Debug.Log("DisplayDeclined2");
