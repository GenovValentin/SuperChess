...
#region  Initialization
    ...
    private void HandleMouseButtonUp(Vector2Int hitPosition)
    {
        if (currentlyDragging != null)
        {
            ...
            {
                MoveTo(currentlyDragging.Position.x,
                currentlyDragging.Position.y,
                hitPosition.x,
                hitPosition.y);

                if (specialMove != SpecialMove.Promotion)
                {
                    SendMoveToServer(currentlyDragging.Position, hitPosition);
                }
            }
            else
            {
                currentlyDragging
                    .SetPosition(GetTileCenter(currentlyDragging.Position.x,
                    currentlyDragging.Position.y));
            }
