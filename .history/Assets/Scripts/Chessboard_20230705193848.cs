...
#region  Initialization
    ...
    // private Color32 whiteColor = new Color32(233, 227, 211, 255);
    // private Color32 blackColor = new Color32(50, 50, 50, 255);
    // private Vector3 whiteNamePosition = new Vector3(-675, 475, 0);
    // private Vector3 blackNamePosition = new Vector3(800, 475, 0);
    // Multiplayer logic
    ...
    private void SetPlayerName(string playerNameInput)
    {
        ...
        Debug.Log("Input Field Edited: " + playerName);
    }
    ...
    private void SetInGamePlayerName(Team thisTeam)
    {
        if (thisTeam == Team.White)
        {
            ...
            // playerNameTMP.color = whiteColor;
            // playerNameTMP.transform.position = whiteNamePosition;
            // Debug.Log("WhiteName " + playerNameTMP.transform.position);
        }
        else if (thisTeam == Team.Black)
        {
            blackPlayerNameTMP.text = playerName;

            // playerNameTMP.color = blackColor;
            // playerNameTMP.transform.position = blackNamePosition;
            // Debug.Log("BlackName " + playerNameTMP.transform.position);
        }
        SendGetOpponentNameToServer (currentTeam);
    }
    ...
    private void SendGetOpponentNameToServer(Team team, string opponentName)
    {
        NetGetOpponentName getOpponentName = new NetGetOpponentName();
        getOpponentName.teamNUMBER = (int) team;
        getOpponentName.opponentName = opponentName;
    ...
    {
        Debug.Log("MyTeam " + myTeam + " " + isWhiteTurn);
    ...
    public void GameReset()
    {
        ...
        if (!localGame)
        {
            Debug.Log("!localGame " + currentTeam);
    ...
    private void ProcessPromotion()
    {
        ...
        PromotePawnToQueen(Team.White, targetPiece, lastMove);
        PromotePawnToQueen(Team.Black, targetPiece, lastMove);
    }
    ...
    private void PromotePawnToQueen(
        Team team,
    ...
    )
    {
        ...
        Debug.Log("Promoting " + lastMove.x + lastMove.y);
        ChessPiece lastPiece = GetLastPiece();
        Debug.Log("LastPiece " + lastPiece.currentX + lastPiece.currentY);
        ChessPiece newQueen = SpawnSinglePiece(ChessPieceType.Queen, team);

        newQueen.transform.position = lastPiece.transform.position;
...
#region
    ...
    {
        Debug.Log("ChangeTeam " + currentTeam);
        currentTeam = GetOppositeTeam(currentTeam);
        ChangeCameraAngles (currentTeam);
        Debug.Log("EndChangeTeam " + currentTeam);
    }
    ...
    private void ActivateDrawIndicator(bool oppWantsDraw)
    {
        ...
        Debug.Log("Buttons.interactable");
    ...
    private void OnRematchClient(NetMessage msg)
    {
        ...
        Debug.Log("OnRematchClient " + rm.teamId + " " + currentTeam);
    ...
    private void OnDeclineClient(NetMessage msg)
    {
        ...
        {
            Debug.Log("Declined " + dc.teamNr + currentTeam);
            ShowDeclined();
